...
module interpolator_8x_tb #(
    ...
    initial begin
        clk = 1'b0;
        file_handle = $fopen(
            "sim/vectors/generated/interpolator_8x_stream.txt", "r"
        );
        if (file_handle == 0)
            $fatal(1, "cannot open 8x interpolation vectors");
        ...
        if (saturation_count != 0 || overrun_count != 0
            ...
            $display(
                "MISMATCH diagnostics saturation=%0d overrun=%0d phase=%0d",
        ...
        $display("PASS: %0d exact 8x interpolation outputs", output_index);
        $finish;
    end
endmodule
