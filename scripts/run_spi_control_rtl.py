#!/usr/bin/env python3
"""Lint and verify the oversampled SPI-to-control-bus transport."""
...
def main() -> int:
    ...
    sources = [
        ...
        "sim/integration/spi_control_transport_tb.sv",
    ]
    common = [
        "--timing", "-Wall", "-Wno-fatal", "-sv",
        "--top-module", "spi_control_transport_tb", *sources,
    ]
    subprocess.run([verilator, "--lint-only", *common], cwd=ROOT, check=True)
    build = ROOT / "build" / "verilator_spi_control_transport"
    subprocess.run(
    ...
    subprocess.run(
        [str(build / "Vspi_control_transport_tb")], cwd=ROOT, check=True
    )
    return 0
