...
.PHONY: audio-clock-plan nexys-audio-top-contract audio-serial-clock-rtl i2c-write-rtl i2c-read-rtl adau1761-codec-init-rtl pcm5242-dac-init-rtl pcm5242-dac-verify-rtl pcm5242-dac-runtime-rtl pcm5242-dac-startup-rtl dac-line-sequencer-rtl codec-shared-i2s-guard-rtl synth-audio-clock-xc7 synth-audio-serial-clock-xc7 synth-i2c-write synth-i2c-read synth-adau1761-codec-init synth-pcm5242-dac-init synth-pcm5242-dac-verify synth-pcm5242-dac-runtime synth-pcm5242-dac-startup synth-dac-line-sequencer synth-codec-shared-i2s-guard synth-nexys-phono-audio-xc7 openxc7-a200t-audio-clock-pnr openxc7-a200t-audio-clock-bit openxc7-a200t-phono-audio-pnr openxc7-a200t-phono-audio-bit nexys-audio-hardware-preflight nexys-audio-program-sram
...
pcm5242-dac-init-rtl:
	$(PYTHON) scripts/run_pcm5242_dac_init_rtl.py --verilator $(VERILATOR)

pcm5242-dac-verify-rtl:
	$(PYTHON) scripts/run_pcm5242_dac_verify_rtl.py --verilator $(VERILATOR)

pcm5242-dac-runtime-rtl:
	$(PYTHON) scripts/run_pcm5242_dac_runtime_monitor_rtl.py --verilator $(VERILATOR)

pcm5242-dac-startup-rtl:
	$(PYTHON) scripts/run_pcm5242_dac_startup_controller_rtl.py --verilator $(VERILATOR)

dac-line-sequencer-rtl:
	$(PYTHON) scripts/run_dac_line_output_sequencer_rtl.py --verilator $(VERILATOR)
...
pcm4202-capture-rtl:
	$(PYTHON) scripts/run_pcm4202_i2s_capture_rtl.py --verilator $(VERILATOR)
...
synth-pcm4202-capture:
	$(PYTHON) scripts/run_synthesis.py --top pcm4202_i2s_capture
...
synth-pcm5242-dac-init:
	$(PYTHON) scripts/run_synthesis.py --top pcm5242_dac_init

synth-pcm5242-dac-verify:
	$(PYTHON) scripts/run_synthesis.py --top pcm5242_dac_verify

synth-pcm5242-dac-runtime:
	$(PYTHON) scripts/run_synthesis.py --top pcm5242_dac_runtime_monitor

synth-pcm5242-dac-startup:
	$(PYTHON) scripts/run_synthesis.py --top pcm5242_dac_startup_controller

synth-dac-line-sequencer:
	$(PYTHON) scripts/run_synthesis.py --top dac_line_output_sequencer
...
nexys-audio-hardware-preflight:
	$(PYTHON) scripts/nexys_video_hardware_preflight.py --bitstream $(NEXYS_AUDIO_BIT) --programmer $(OPENFPGALOADER) --probe-hardware

nexys-audio-program-sram:
	$(PYTHON) scripts/nexys_video_hardware_preflight.py --bitstream $(NEXYS_AUDIO_BIT) --programmer $(OPENFPGALOADER) --require-hardware
	$(OPENFPGALOADER) -b nexysVideo $(NEXYS_AUDIO_BIT)
