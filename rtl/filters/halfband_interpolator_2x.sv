...
module halfband_interpolator_2x #(
    ...
    always_comb begin
        product = coefficient[tap_index] * history[tap_index[INDEX_WIDTH-1:1]];
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            tap_index <= '0;
            accumulator <= '0;
            for (lane = 0; lane < INPUT_HISTORY; lane = lane + 1)
                history[lane] <= '0;
        end else begin
            output_valid <= 1'b0;
            if (ce_output) begin
