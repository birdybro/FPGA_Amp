...
module phono_stream_mono_tb #(
    ...
    logic [54:0] solver_last_residual_q44;
    ...
    initial begin
        ...
        for (output_index = 0; output_index < VECTOR_COUNT; output_index = output_index + 1) begin
        ...
        if (resampler_saturation_count != 0 || resampler_overrun_count != 0 ||
            input_phase_error_count != 0 || output_conversion_saturation_count != 0 ||
            solver_missed_request_count != 0 || solver_deadline_miss_count != 0 ||
            solver_saturation_count != 0 || solver_lut_clip_count != 0 ||
            solver_nonconvergence_count != 0 || solver_latency_cycles != 8'd126 ||
            solver_last_residual_q44 > 55'd35184372) begin
            $display(
                "MISMATCH diagnostics rsat=%0d rover=%0d phase=%0d convsat=%0d smissed=%0d sdeadline=%0d ssat=%0d sclip=%0d snonconv=%0d latency=%0d residual=%0d",
                ...
                solver_nonconvergence_count, solver_latency_cycles,
        ...
        $display(
            "PASS: %0d exact 48 kHz phono outputs, solver latency=%0d clocks",
            output_index, solver_latency_cycles
        );
        $finish;
    end
endmodule
