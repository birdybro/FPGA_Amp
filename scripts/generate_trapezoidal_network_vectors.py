...
def main() -> int:
    ...
    cap_g_path = (
        generated / f"v1_cap_conductance_q0_47_trapezoidal{rate_suffix}.mem"
    )
    cap_g_header_path = (
        generated / f"v1_cap_conductance_q0_47_trapezoidal{rate_suffix}.svh"
    )
    ...
    if args.sample_rate_hz == 768_000 and max(conductance) < (1 << 46):
    ...
    terminal_function_name = (
        "v1_terminal_cap_g_q47"
        if args.sample_rate_hz == 768_000
        else "v1_terminal_cap_g_q47_384khz"
    )
    cap_g_header_path.write_text(
        "\n".join(
            [
                "// Generated by scripts/generate_trapezoidal_network_vectors.py.",
                "// Frozen V1 trapezoidal capacitor conductances, signed Q0.47.",
                f"// Internal sample rate: {args.sample_rate_hz} Hz.",
                "function automatic logic signed [47:0] "
                f"{terminal_function_name}(",
                "    input int index",
                ");",
                "    begin",
                "        case (index)",
                *[
                    f"            {index}: {terminal_function_name} = 48'sd{value};"
                    for index, value in enumerate(conductance)
                ],
                f"            default: {terminal_function_name} = '0;",
                "        endcase",
                "    end",
                "endfunction",
                "",
    ...
    vector_path = (
        ...
        / "vectors"
        / "generated"
        / f"network_kcl_wide_trapezoidal{rate_suffix}.txt"
    )
    ...
    metadata = generated / f"wide_network_trapezoidal{rate_suffix}_metadata.json"
