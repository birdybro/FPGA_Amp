...
- A separately labeled 384 kHz/8× trapezoidal architecture candidate measures
  ...
  in-band. A layer decomposition measures the identity converters at -67.49 dB
  in-band, floating Newton at -55.71 dB, and fixed at -53.33 dB; same-rate
  ...
  reaches only 47.567 MHz through the now-exposed chord-to-tube path.
  Registering the six raw tube nodes on the exact one-cycle-early chord preview
  removes that path without adding a solver clock: the same complete stream is
  bit-exact at 127 clocks, Yosys measures 15,220 LC / 8,855 FF / 207 DSP /
  10 RAMB18 equivalents, and a legal seed-1 A200T route reaches 61.072 MHz
  against 49.152 MHz. The 16.37 ns critical path is now terminal-current/state
  arithmetic, with 6.44 ns logic and 9.94 ns routing. This closes the target
...
placement to 33.92 MHz, although its router2 attempt did not converge legally.
...
or fractional delay. `--gain-align` and `--fractional-delay` are opt-in. The
fractional method is explicitly selected as `linear` or `windowed_sinc`, and a
known physical delay can be supplied instead of correlation. Every applied
transformation is recorded alongside the unaligned metrics.
...
The matching front-panel controller is at
[`hardware/kicad/front_panel_controller_rev_a/`](hardware/kicad/front_panel_controller_rev_a/README.md).
