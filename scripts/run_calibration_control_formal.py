#!/usr/bin/env python3
"""Formally prove atomic converter-calibration commit behavior."""
...
PREPARE = (
    ...
    "sim/formal/calibration_commit_guard_formal.sv; "
    "prep -top calibration_commit_guard_formal; "
    "flatten; "
    "async2sync; "
    "formalff -clk2ff; "
    "opt_clean"
...
def main() -> int:
    ...
    build = REPOSITORY_ROOT / "build" / "formal_calibration_commit_guard"
    ...
    if "Induction step proven: SUCCESS!" not in proof:
        print("ERROR: calibration temporal induction did not pass", file=sys.stderr)
        return 1
    ...
    # Find one trace containing invalid rejection, an accepted atomic pair, and
    # unsafe rejection. The final active pair and both stickies make all three
    # paths observable without constraining the intermediate input choices.
    reachability_command = (
        f"{PREPARE}; "
        "sat -set-assumes -set-init-zero -seq 6 "
        "-set-at 6 active_input_peak_q24 1 "
        "-set-at 6 active_output_reciprocal_q24 1 "
        "-set-at 6 invalid_update_sticky 1 "
        "-set-at 6 unsafe_update_sticky 1 "
        "-show active_input_peak_q24 "
        "-show active_output_reciprocal_q24 "
        "-show invalid_update_sticky -show unsafe_update_sticky"
    )
    try:
    ...
    if "SAT solving finished - model found:" not in reachability:
        print("ERROR: calibration path witness was not found", file=sys.stderr)
        return 1

    print(
        "PASS: calibration guard 12-property temporal induction depth "
        f"{induction_depth}; reject/commit/reject witness found"
    )
    return 0
