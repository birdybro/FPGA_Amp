#!/usr/bin/env python3
"""Generate, lint, and verify the register-controlled pin-facing hierarchy."""
...
def main() -> int:
    ...
    sources = [
        ...
        "sim/integration/phono_i2s_control_top_tb.sv",
    ]
    common = [
        "--timing",
        "-Wall",
    ...
    build = ROOT / "build" / f"verilator_phono_i2s_control_top{rate_suffix}"
    subprocess.run(
    ...
    subprocess.run(
        [str(build / "Vphono_i2s_control_top_tb")], cwd=ROOT, check=True
    )
    return 0
